...
class UserManager(TenantManager, BaseUserManager):
    ...
    def create_user(self, tenant_id, username, email, password=None, **extra_fields):
        ...
        user = self.model(tenant_id=tenant_id, username=username, email=email, **extra_fields)
...
class User(AbstractBaseUser, PermissionsMixin, TenantIsolatedModel):
    ...
    tenant = models.ForeignKey("tenants.Tenant", on_delete=models.CASCADE, related_name="users", db_column="tenant_id")
...
class APIKey(TenantIsolatedModel):
    ...
    tenant = models.ForeignKey(
        "tenants.Tenant", on_delete=models.CASCADE, related_name="api_keys", db_column="tenant_id"
    )
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="api_keys", db_column="user_id")
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["key_hash"], name="idx_api_keys_hash", condition=models.Q(revoked=False)),
        ]
