...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for tenant in pending_tenants:
            ...
            deletion_log = AuditLog.objects.filter(
                tenant_id=tenant.id,
        ...
        if not tenants_to_delete:
            self.stdout.write(
                self.style.SUCCESS('No tenants ready for deletion')
            )
            return
        
        self.stdout.write(
            f'Found {len(tenants_to_delete)} tenants ready for deletion:'
        )
            ...
            try:
                with transaction.atomic():
                    ...
                    AuditLog.objects.create(
                        ...
                        details={
                            'tenant_id': tenant.id,
                            'deletion_completed_at': timezone.now().isoformat()
                        }
                    )
                    ...
                    deleted_count += 1
                    self.stdout.write(
                        self.style.SUCCESS(f'Deleted tenant: {tenant.id}')
                    )
                    
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f'Failed to delete tenant {tenant.id}: {e}')
                )
            finally:
                clear_current_tenant()
        
        self.stdout.write(
            self.style.SUCCESS(f'Successfully deleted {deleted_count} tenants')
        )