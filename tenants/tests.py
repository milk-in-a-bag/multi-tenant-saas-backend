...
class TenantManagerTest(TestCase):
    ...
    def test_register_tenant_success(self):
        ...
        result = TenantManager.register_tenant(
            identifier="test-company", admin_email="admin@test.com", admin_username="admin"
        )
        ...
        audit_log = AuditLog.objects.get(tenant_id="test-company", event_type="tenant_registered")
    ...
    def test_register_duplicate_tenant_fails(self):
        ...
        # Create first tenant
        TenantManager.register_tenant(identifier="test-company", admin_email="admin@test.com")

        # Try to create duplicate
        with self.assertRaises(ValidationError) as context:
            TenantManager.register_tenant(identifier="test-company", admin_email="admin2@test.com")
    ...
    def test_update_subscription(self):
        ...
        TenantManager.update_subscription(tenant_id="test-company", tier="professional", expiration_date=new_expiration)
        ...
        # Check audit log
        audit_log = AuditLog.objects.get(tenant_id="test-company", event_type="subscription_updated")
    ...
    def test_delete_tenant_marks_pending(self):
        ...
        # Create tenant
        result = TenantManager.register_tenant(identifier="test-company", admin_email="admin@test.com")

        # Get admin user
        admin_user = User.objects.get(tenant_id="test-company", role="admin")

        # Delete tenant
        TenantManager.delete_tenant(
            tenant_id="test-company", admin_user_id=admin_user.id, password=result["admin_password"]
        )
        ...
        # Check audit log
        audit_log = AuditLog.objects.get(tenant_id="test-company", event_type="tenant_deletion_requested")
    ...
    def test_check_pending_deletion_status(self):
        ...
        self.assertTrue(TenantManager.check_pending_deletion_status("test-company"))
        ...
        self.assertFalse(TenantManager.check_pending_deletion_status("test-company"))
