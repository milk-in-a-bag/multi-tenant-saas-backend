...
class TenantManager:
    ...
    def register_tenant(identifier, admin_email, admin_username=None):
        ...
        if Tenant.objects.filter(id=identifier).exists():
            raise DRFValidationError({"identifier": "A tenant with this identifier already exists"})
        ...
        try:
            with transaction.atomic():
                ...
                Tenant.objects.create(
                    id=identifier, subscription_tier="free", subscription_expiration=default_expiration, status="active"
                )
                ...
                return {"tenant_id": identifier, "admin_username": admin_username, "admin_password": temp_password}
    ...
    def delete_tenant(tenant_id, admin_user_id, password):
        ...
        try:
            ...
            try:
                ...
                raise DRFValidationError({"error": "Admin user not found or insufficient permissions"})

            # Verify password for re-authentication
            if not admin_user.check_password(password):
                raise DRFValidationError({"password": "Invalid password for re-authentication"})
            ...
            with transaction.atomic():
                ...
                try:
                    ...
                    raise DRFValidationError({"tenant_id": "Tenant not found"})
    ...
    def update_subscription(tenant_id, tier, expiration_date):
        ...
        if tier not in valid_tiers:
            raise DRFValidationError({"tier": f"Invalid subscription tier. Must be one of: {valid_tiers}"})
    ...
    def get_tenant_config(tenant_id):
        ...
        if tenant.subscription_expiration < timezone.now() and tenant.subscription_tier != "free":

            # Auto-downgrade expired subscription
            TenantManager.update_subscription(tenant_id, "free", tenant.subscription_expiration)
            tenant.refresh_from_db()

        # Get rate limit based on subscription tier
        rate_limits = {"free": 100, "professional": 1000, "enterprise": 10000}
