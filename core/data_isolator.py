...
class DataIsolator:
    ...
    def write(sql, params=None):
        ...
        """
        DataIsolator.validate_tenant_context()

        with connection.cursor() as cursor:
            cursor.execute(sql, params)
            return {"rowsAffected": cursor.rowcount, "insertedId": cursor.lastrowid if cursor.lastrowid else None}
...
class TenantIsolatedModel(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if obj_tenant_id and tenant_id and str(obj_tenant_id) != str(tenant_id):
            raise TenantIsolationError(
                f"Cannot save object with tenant_id {obj_tenant_id} " f"in context of tenant {tenant_id}"
            )
    ...
    def delete(self, *args, **kwargs):
        ...
        if obj_tenant_id and tenant_id and str(obj_tenant_id) != str(tenant_id):
            raise TenantIsolationError(
                f"Cannot delete object with tenant_id {obj_tenant_id} " f"in context of tenant {tenant_id}"
            )
