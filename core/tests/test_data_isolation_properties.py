...
class TestTenantDataIsolationProperties(TestCase):
    ...
    @given(
        tenants_data=st.lists(
            tenant_with_data_strategy(),
            min_size=2,
            max_size=5,
            unique_by=lambda x: x['tenant_id']
        ),
    ...
    def test_property_9_queries_return_only_tenant_data(self, tenants_data, querying_tenant_index):
        ...
        **Validates: Requirements 3.1, 3.3**
        ...
        try:
            ...
            for log in results:
                assert log.tenant_id == querying_tenant_id, (
            ...
            for other_tenant_id in other_tenant_ids:
                assert other_tenant_id not in result_tenant_ids, (
            ...
            expected_count = next(
                t['num_logs'] for t in tenants_data 
                if t['tenant_id'] == querying_tenant_id
            )
            assert len(results) == expected_count, (
                f"Expected {expected_count} logs for tenant {querying_tenant_id}, "
