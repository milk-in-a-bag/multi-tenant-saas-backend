...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter('name_contains', str, OpenApiParameter.QUERY,
                         description='Filter by name substring (case-insensitive)', required=False),
        OpenApiParameter('created_after', str, OpenApiParameter.QUERY,
                         description='Return widgets created after this datetime (ISO 8601)', required=False),
        OpenApiParameter('created_before', str, OpenApiParameter.QUERY,
                         description='Return widgets created before this datetime (ISO 8601)', required=False),
        OpenApiParameter('page', int, OpenApiParameter.QUERY,
                         description='Page number (default: 1)', required=False),
        OpenApiParameter('page_size', int, OpenApiParameter.QUERY,
                         description='Results per page (default: 20, max: 100)', required=False),
    ],
    responses={
...
@extend_schema(
    ...
    description=(
        ...
        'The widget must belong to the authenticated tenant.'
        + _TENANT_ISOLATION_NOTE
        + _RATE_LIMIT_NOTE
    ),
    responses={
...
@extend_schema(
    ...
    description=(
        ...
        'Use PATCH for partial updates. Requires admin or user role.'
        + _TENANT_ISOLATION_NOTE
        + _RATE_LIMIT_NOTE
    ),
...
@extend_schema(
    ...
    description=(
        'Update one or more widget fields. Requires admin or user role.'
        + _TENANT_ISOLATION_NOTE
        + _RATE_LIMIT_NOTE
    ),
    request=WidgetUpdateSerializer,
...
@extend_schema(
    ...
    description=(
        'Permanently delete a widget. Requires admin or user role.'
        + _TENANT_ISOLATION_NOTE
        + _RATE_LIMIT_NOTE
    ),
    responses={
